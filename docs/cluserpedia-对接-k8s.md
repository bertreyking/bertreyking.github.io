# cluserpedia 对接 k8s 集群

## 1. 接入集群中创建 clusterrole/clusterrolebinding/serviceaccout/secret 

```shell
# 创建 cluserpedia-RBAC
[root@master01 clusterpedia]# vi clusterpedia_synchro_rbac.yaml 
apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRole
metadata:
  name: clusterpedia-synchro
rules:
- apiGroups:
  - '*'
  resources:
  - '*'
  verbs:
  - '*'
- nonResourceURLs:
  - '*'
  verbs:
  - '*'
---
apiVersion: v1
kind: ServiceAccount
metadata:
  name: clusterpedia-synchro
  namespace: default
---
apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRoleBinding
metadata:
  name: clusterpedia-synchro
roleRef:
  apiGroup: rbac.authorization.k8s.io
  kind: ClusterRole
  name: clusterpedia-synchro
subjects:
- kind: ServiceAccount
  name: clusterpedia-synchro
  namespace: default
---
[root@master01 clusterpedia]# kubectl apply -f clusterpedia_synchro_rbac.yaml 
clusterrole.rbac.authorization.k8s.io/clusterpedia-synchro created
serviceaccount/clusterpedia-synchro created
clusterrolebinding.rbac.authorization.k8s.io/clusterpedia-synchro created

# 查看 sa/secret
[root@master01 clusterpedia]# kubectl get sa 
NAME                   SECRETS   AGE
clusterpedia-synchro   1         9s
default                1         2d14h
[root@master01 clusterpedia]# kubectl get secret 
NAME                               TYPE                                  DATA   AGE
clusterpedia-synchro-token-bjj5m   kubernetes.io/service-account-token   3      14s
default-token-jxtkh                kubernetes.io/service-account-token   3      2d14h
sh.helm.release.v1.dao-2048.v1     helm.sh/release.v1                    1      43h

# 获取 caData/toeknData
[root@master01 clusterpedia]# kubectl get secret clusterpedia-synchro-token-bjj5m -o yaml
apiVersion: v1
data:
  ca.crt: 
  namespace: ZGVmYXVsdA==
  token: 
kind: Secret
metadata:
  annotations:
    kubernetes.io/service-account.name: clusterpedia-synchro
    kubernetes.io/service-account.uid: 61aa6635-9e4e-490c-af7b-d45e98dc49ae
  creationTimestamp: "2023-01-11T02:07:32Z"
  name: clusterpedia-synchro-token-bjj5m
  namespace: default
  resourceVersion: "1032452"
  selfLink: /api/v1/namespaces/default/secrets/clusterpedia-synchro-token-bjj5m
  uid: 52f797f6-9c86-4ac4-935e-7d9ce5f87370
type: kubernetes.io/service-account-token
```

## 2. cluserpedia 组件节点中创建 pediacluser 实例

```shell
# 创建 pediacluser 配置文件
[root@master01 ~]# cat clusterpedia/examples/pediacluster-xxxx.yaml 
apiVersion: cluster.clusterpedia.io/v1alpha2
kind: PediaCluster
metadata:
  name: cluster2-xxxx
spec:
  apiserver: "https://10.x.x.27:16443"
  caData: ""
  tokenData: ""
  syncResources:
  - group: apps
    resources:
     - deployments
  - group: ""
    resources:
     - pods
     - configmaps
  - group: cert-manager.io
    versions:
      - v1
    resources:
      - certificates

# caData / tokenData 均来自接入集群中 clusterpedia-synchro-token-bjj5m Secret 无需进行 base64 解码。另外 1.2x 版本的 k8s，在创建 sa 后不会自动创建对应的 secret，需手动创建 sa 类型的 secret

# 查看 pediacluser 列表
[root@master01 ~]# kubectl get pediacluster
NAME                 READY   VERSION    APISERVER
cluster1-xxxx   True    v1.24.7    https://10.x.x.x:6443
cluster2-xxxx   True    v1.18.20   https://10.x.x.27:16443

# 通过 -o yaml 可以查看资源的同步状态
```

## 3. cluserpedia 组件节点中为 kubectl 生成集群快捷访问配置

```shell
# 备份 kubeconfig 文件
[root@master01 ~]# cp .kube/config .kube/config_bak

# 追加 pediacluser 配置信息到 kubeconfig
[root@master01 ~]# bash gen_clusterconfig.sh 
Current Context: kubernetes-admin@cluster.local
Current Cluster: cluster.local
        Server: https://127.0.0.1:6443
        TLS Server Name: 
        Insecure Skip TLS Verify: 
        Certificate Authority: 
        Certificate Authority Data: ***

Cluster "clusterpedia" set.
Cluster "cluster1-xxxx" set.
Cluster "cluster2-xxxx" set.

# 执行后，会在 .kube/config 新增 3 个 cluser 配置，分别是 get pediacluser 集群信息、和 1 个 clusterpedia <这个是属于一个多集群的概念，通过它可以检索所有集群的资源信息>
```

## 4. cluserpedia 组件节点中使用 kubectl 检查集群资源 [更多检索技巧](https://clusterpedia.io/zh-cn/docs/usage/search/)

```shell
# 检索所有 pediacluster 所同步的资源信息
[root@master01 ~]# kubectl --cluster clusterpedia api-resources
NAME          SHORTNAMES   APIVERSION   NAMESPACED   KIND
configmaps    cm           v1           true         ConfigMap
pods          po           v1           true         Pod
deployments   deploy       apps/v1      true         Deployment

# 检索所有 pediacluster 资源
[root@master01 ~]# kubectl --cluster clusterpedia get deployments.apps -A 
NAMESPACE             CLUSTER              NAME                                                 READY   UP-TO-DATE   AVAILABLE   AGE
calico-apiserver      cluster1-xxxx        calico-apiserver                                     1/1     1            1           73d
clusterpedia-system   cluster1-xxxx        clusterpedia-apiserver                               1/1     1            1           33d
clusterpedia-system   cluster1-xxxx        clusterpedia-clustersynchro-manager                  1/1     1            1           33d
dce-system            cluster2-xxxx        dce-system-dnsservice                                1/1     1            1           47h
dce-system            cluster2-xxxx        dce-system-loadbalancermanager                       1/1     1            1           44h
dce-system            cluster2-xxxx        dce-system-uds                                       1/1     1            1           2d14h
default               cluster2-xxxx        dao-2048                                             1/1     1            1           43h
kube-system           cluster2-xxxx        calico-kube-controllers                              1/1     1            1           2d14h
kube-system           cluster2-xxxx        coredns-coredns                                      2/2     2            2           47h
kube-system           cluster2-xxxx        dce-chart-manager                                    1/1     1            1           2d14h
kube-system           cluster2-xxxx        dce-clair                                            1/1     1            1           2d14h

# 检索指定集群和租户信息
[root@master01 ~]# kubectl --cluster cluster2-xxxx get deployments.apps -n kube-system 
CLUSTER            NAME                                                 READY   UP-TO-DATE   AVAILABLE   AGE
cluster2-xxxx   calico-kube-controllers                              1/1     1            1           2d14h
cluster2-xxxx   coredns-coredns                                      2/2     2            2           47h
cluster2-xxxx   dce-chart-manager                                    1/1     1            1           2d14h
cluster2-xxxx   dce-clair                                            1/1     1            1           2d14h
cluster2-xxxx   dce-controller                                       1/1     1            1           2d14h
cluster2-xxxx   dce-core-keepalived                                  1/1     1            1           2d14h
cluster2-xxxx   dce-prometheus                                       1/1     1            1           2d14h
cluster2-xxxx   dce-registry                                         1/1     1            1           2d14h
cluster2-xxxx   dce-uds-daocloud-dlocal-local-1-0-0-csi-controller   1/1     1            1           37h
cluster2-xxxx   dce-uds-failover-assistant                           1/1     1            1           2d14h
cluster2-xxxx   dce-uds-policy-controller                            1/1     1            1           2d14h
cluster2-xxxx   dce-uds-snapshot-controller                          1/1     1            1           2d14h
cluster2-xxxx   dce-uds-storage-server                               1/1     1            1           2d14h
cluster2-xxxx   lb01-ingress1                                        1/1     1            1           44h
cluster2-xxxx   lb01-ingress2                                        1/1     1            1           44h
cluster2-xxxx   lb01-keepalived                                      2/2     2            2           44h
cluster2-xxxx   metrics-server                                       1/1     1            1           2d14h

[root@master01 ~]# kubectl --cluster cluster2-xxxx get pod -o wide -n kube-system 
CLUSTER            NAME                                                              READY   STATUS    RESTARTS        AGE     IP               NODE       NOMINATED NODE   READINESS GATES
cluster2-xxxx   calico-kube-controllers-789ff96b77-lwvb6                          1/1     Running   0               2d14h   10.x.x.27      master01   <none>           <none>
cluster2-xxxx   calico-node-28xtc                                                 1/1     Running   0               2d13h   10.x.x.33      worker02   <none>           <none>
cluster2-xxxx   calico-node-bc8b7                                                 1/1     Running   0               2d14h   10.x.x.27      master01   <none>           <none>
cluster2-xxxx   calico-node-bs9fh                                                 1/1     Running   0               2d13h   10.x.x.30      worker01   <none>           <none>
cluster2-xxxx   calico-node-dbz46                                                 1/1     Running   0               37h     10.x.x.39      worker03   <none>           <none>
cluster2-xxxx   coredns-coredns-5b879856b7-lnj4g                                  1/1     Running   0               47h     172.28.2.130     worker01   <none>           <none>
cluster2-xxxx   coredns-coredns-5b879856b7-mjjcl                                  1/1     Running   0               47h     172.28.48.1      worker02   <none>           <none>
cluster2-xxxx   dce-chart-manager-795cdfd86b-6dhnv                                1/1     Running   0               2d14h   172.28.213.70    master01   <none>           <none>
```

